...
// Ravenrift: the classic walled field is 100x280yd (diagonal ~297yd) and
// stays fully tracked for its own match, so the whole battle exists in every
// participant's sim mirror (the CLIENT hides past ~120yd behind the band's
// distance fog, like the open world's view distance). Applies to SAME-SLOT
// pairs only: slot spacing (BG_SLOT_SPACING, 460) keeps cross-slot pairs
// beyond every radius here.
