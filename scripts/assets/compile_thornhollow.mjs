...
for (const p of placements) {
  ...
  const base = seatGround(p) + (p.y ?? 0);
  // Volumes default to camGhost (the chase cam ignores invisible helpers), but
  // a volume standing in for a REAL wall opts into camera solidity with
  // camSolid, and camTopY overrides the derived occlusion/sight top (a pillar
  // dressed tall occludes at its dressed height, not its footprint).
  if (kind === 'sphere') {
    colliders.push({
      type: 'circle',
      x: round(p.x),
      z: round(p.z),
      r: round(sx / 2),
      cameraTopY: round(base + (p.camTopY ?? sx)),
      camGhost: p.camSolid !== true,
    });
  } else {
    pushObb(p.x, p.z, sx / 2, sz / 2, p.rotY ?? 0, {
      cameraTopY: round(base + (p.camTopY ?? sy)),
      camGhost: p.camSolid !== true,
    });
  }
}
...
const probes = [];
// Scatter stays 2yd inside the document rect: the baked grid clamps at the
// rect edge, so a probe outside it would compare a clamped read against the
// unclamped chain and pin a fiction. (118/224 for the 120x226 Thornhollow.)
const PROBE_HALF_X = Math.min(118, HALF_X - 2);
const PROBE_HALF_Z = Math.min(224, HALF_Z - 2);
for (let i = 0; i < 40; i++) {
...
lines.push(`// Source: data/battleground/thornhollow.map.json (map id ${map.meta?.id ?? 'unknown'}).`);
lines.push('// Data only: no imports beyond types, no runtime work at module load.');
...
lines.push('');
// The three big tables are emitted as JSON.parse over a string literal: a
// typed object-literal array this large trips tsc's union-complexity limit
// (TS2590), while a parsed constant is assignability-checked only against the
// declared type. The JSON never contains quotes or backslashes (keys and
// numbers only), so the single-quoted embedding needs no escaping.
const emitJsonConst = (name, type, value) => {
  const s = JSON.stringify(value);
  if (s.includes("'") || s.includes('\\')) throw new Error(`${name}: JSON needs escaping`);
  lines.push(`export const ${name}: ${type} = JSON.parse(`);
  lines.push(`  '${s}',`);
  lines.push(');');
};
